...
import { ORDER_DETAILS_ROUTE, EDIT_ORDER_ROUTE, ORDER_STATUS_TYPES, ORDER_EMIRATES } from '../../constants/index';
...
function ListOrders({ clientOrders }) {
  ...
  const handleChooseDriver = () => {
    ...
    promiseDrivers.then((response) => {
      setDrivers(response.data.result.result.filter(item => item.status !== 2))
  ...
  return (
    <div >
          ...
          <TableBody>
            {(clientOrders?clientOrders.orders:Orders.orders).map((item, index) => (
              <TableRow
                ...
                >
                  <Button
                    className={item.driverName ? classes.chooseDriverButton1 : classes.chooseDriverButton2}
                    onClick={() => {
                      setItemId(index);
                      handleChooseDriver();
                    }}>
                    {item.driverName ? item.driverName : "Choose driver"}
                  </Button>
                </TableCell>
                <TableCell
                  align="left"
                ...
                <TableCell
                  ...
                  component={Link}
                  to={getDetailsRoute(item.id)}>
                  {ORDER_EMIRATES[item.emirate]}</TableCell>
                <TableCell
                  className={classes.tableCell}
                  align="left"
                  component={Link}
      ...
      <Dialog
        open={chooseDriver}
        onClose={handleCloseChooseDriver}
        aria-labelledby="alert-dialog-title"
        aria-describedby="alert-dialog-description"
      >
        <DialogTitle id="alert-dialog-title">{"Choose driver"}
        </DialogTitle>
        <DialogContent>
          <DialogContentText id="alert-dialog-description">
            {waitingDriver ?
              <div>Please wait</div>
              : drivers.length === 0 ?
                <div>There is not available driver</div>
                :
                <FormControl
                    ...
                    {drivers.map(driver => (
                      <MenuItem
                        ...
                        className={
                          driver.status == 0 ?
                            classes.busyDriver :
                            classes.availableDriver
                        }
                        value={driver.id}
                      >
                        {driver.firstName + " " + driver.lastName + " " + driver.nickName}
                      </MenuItem>
                    ))}
                  </Select>
                </FormControl>}
        ...
        <DialogActions>
          <Button onClick={handleCloseChooseDriver} color="primary">
            Cancel
          </Button>
          <Button
            onClick={() => {
