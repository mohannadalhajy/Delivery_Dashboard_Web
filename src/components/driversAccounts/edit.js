...
import { editDriverAccount, initEditDriverAccount } from '../../redux/driversAccounts/Actions';
import { DRIVERS_ACCOUNTS_ROUTE } from '../../constants';
const APIDriverAccount = require('../../redux/driversAccounts/API');
...
function EditDriverAccount() {
  ...
  useEffect(() => {
    ...
    const promise = APIDriverAccount.getById(id);
    promise.then(res => {
    ...
    const promise1 = APIDriver.getCompaniesNames();
  ...
  const EditClick = (e) => {
    ...
    dispatch(editDriverAccount({
      "id": pos,
      "body": recordState
    }));
  }
  const moveToDetails = () => {
    history.push(DRIVERS_ACCOUNTS_ROUTE)
    dispatch(initEditDriverAccount)
  }
  return (
                ...
                <Grid item xs={12} sm={5}>
                  <FormControl
                    ...
                    <Select
                      ...
                      label="Driver name">
                      {drivers.map(driver => (<MenuItem value={driver.id}>{driver.companyNameEnglish}</MenuItem>))}
                    </Select>
                  </FormControl>
                </Grid>
                <Grid item xs={1} />
