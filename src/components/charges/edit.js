...
import { editCharge, initEditCharge } from '../../redux/charges/Actions';
import { CHARGES_ROUTE } from '../../constants';
const APICharge = require('../../redux/charges/API');
...
function EditCharge() {
  ...
  useEffect(() => {
    ...
    const promise = APICharge.getById(id);
    promise.then(res => {
  ...
  const moveToDetails = () => {
    history.push(CHARGES_ROUTE)
    dispatch(initEditCharge)
  }
  return (
